...
import (
	...
	"game-currency/src/interface/rest/handlers"
	"game-currency/src/interface/rest/response"
	"game-currency/src/interface/rest/route"
...
func makeRoute(
	...
	bh := conversionHandler.NewConversionHandler(respClient, useCases.ConversUseCase)
	ch := currencyHandler.NewCurrencyHandler(respClient, useCases.CurrencyUseCase)

	r.Route("/api", func(r chi.Router) {
		r.Mount("/", route.HealthRouter(hh))
		r.Mount("/conversion", route.ConversionAppRouter(bh))
		r.Mount("/currency", route.CurrencyAppRouter(ch))
